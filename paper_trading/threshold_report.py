...
def load_all_legs():
    ...
    try:
        with open(os.path.join(STATE_DIR, 'momentum_state.json')) as f:
            for p in json.load(f).get('open_positions', []):
                add(p['symbol'], 'v1整點版', None, p['direction'], p['entry_price'], p['sl_price'], p['tp_price'])
    ...
    try:
        with open(os.path.join(STATE_DIR, 'momentum_v7_state.json')) as f:
            for p in json.load(f).get('positions', []):
                add(p['symbol'], 'v7提早進場+重倉版', None, p['direction'], p['entry_price'], p['sl_price'], p['tp_price'])
    except (FileNotFoundError, json.JSONDecodeError):
        pass
    ...
    try:
        with open(os.path.join(STATE_DIR, 'momentum_v8_state.json')) as f:
            for p in json.load(f).get('open_positions', []):
                add(p['symbol'], 'v8快停利版', None, p['direction'], p['entry_price'], p['sl_price'], p['tp_price'])
    except (FileNotFoundError, json.JSONDecodeError):
        pass
