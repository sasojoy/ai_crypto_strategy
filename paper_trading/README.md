...
- `threshold_report.py` — NOT a monitor, doesn't open/close any position; only READS (never writes) the other three monitors' state files. Runs hourly (`PaperTrading-ThresholdReport` below) and sends TWO separate Telegram messages, kept apart because they answer different questions:
