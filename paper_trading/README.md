...
- `threshold_report.py` — NOT a monitor, doesn't open/close any position; only READS (never writes) the other three monitors' state files, purely to annotate its output. Read-only status report added 2026-09-09: reuses `momentum_monitor_v3.py`'s RSI-threshold math (`threshold_price()`) and live-price fetch to compute, for all 5 symbols, the exact price that would push this still-forming hour's RSI to 70 (`thr_long`, the LONG-entry trigger — momentum continuation, NOT mean reversion, so touching the *upper* threshold with volume confirmation means "go long, expect the breakout to continue," not "short the top") or to 30 (`thr_short`, the SHORT-entry trigger). Sorts by whichever threshold each symbol is closest to right now and sends only the nearest 2 to Telegram, hourly, so you don't get all 5 every run. Each reported symbol also gets a `⚠️ 目前已持倉: v1(多)、v2(多)...` / `目前無持倉` line (added 2026-09-09, `load_open_positions()`) so a threshold reading for a symbol you're already holding doesn't read like a brand-new signal is imminent — the RSI-threshold math itself doesn't know about existing positions or any monitor's cooldown, it's purely "what price would flip RSI right now." Scheduled as `PaperTrading-ThresholdReport` below.
