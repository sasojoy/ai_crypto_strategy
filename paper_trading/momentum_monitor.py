...
def main():
    ...
    for pos in state['open_positions']:
        ...
        if closed:
            ...
            msg = (f"📕 【模擬盤出場-v1整點版】{closed['symbol']} {closed['direction'].upper()}\n"
    ...
    for s in SYMBOLS:
        ...
        for i, reversion_direction in triggers:
            ...
            if len(state['open_positions']) >= MAX_CONCURRENT or trial_risk > RISK_BUDGET:
                msg = (f"⏭️ 【訊號略過-v1整點版，相關性風險預算已滿（{trial_risk:.2f} > {RISK_BUDGET}）】{s} "
            ...
            msg = (f"📗 【模擬盤進場-v1整點版】{s} {direction.upper()}（{'超賣' if reversion_direction=='long' else '超買'}動能延續）\n"
