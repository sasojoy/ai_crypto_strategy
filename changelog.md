...
### Added
- **Pooled Training**: Implemented a unified training pipeline that combines features from multiple symbols (BTC, ETH, SOL, BNB, AVAX) into a single training set.
- **Multi-Symbol Backtest**: Updated the backtest engine and strategy main to support portfolio-wide performance evaluation.
- **Portfolio Metrics**: Added combined equity curve, total trades (Target N > 100), and symbol-specific profit factors.

### Changed
- **Strategy Refinement**: 
    - Timeframe: Strictly 1H.
    - Entry Condition: Predicted_Max_Return > 0.8% (next 12h) AND 1H ADX > 20.
- **Risk Management**:
    - Partial Take-Profit: Close 50% at 1.0 * ATR.
    - Hard Stop-Loss: 1.2 * ATR.
    - Move Stop to Entry + 0.0009 (fees) once TP1 is hit.
- **Config Update**: Expanded symbol list and added `max_allocation_per_trade` (1.0x).
