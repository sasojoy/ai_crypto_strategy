...
def main():
    ...
    while True:
        for symbol in symbols:
            try:
                now = datetime.now()
                print(f"[{now.strftime('%Y-%m-%d %H:%M:%S')}] 正在分析 {symbol}...")
                ...
                # 15 分鐘定時回報 (Heartbeat)
                if symbol not in last_heartbeat or (now - last_heartbeat[symbol]).total_seconds() >= 900:
                    heartbeat_msg = (
                        f"📊 [目標 100 萬] 定時狀態回報\n"
                        f"----------------------------\n"
                        f"幣種：{symbol}\n"
