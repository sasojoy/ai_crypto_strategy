...
def run_strategy(ml_model):
    ...
    # Iteration 91.1.3: Data Pre-warmup & Progress Tracking (Forced)
    warmup_count = 0
    total_symbols = len(symbols)
    for i, s in enumerate(symbols):
        # Pre-fetch data to ensure indicators are ready
        df_warmup = fetch_15m_data(s)
        # Iteration 91.1.3: Require 500 candles for full indicator readiness
    ...
    if warmup_count < total_symbols:
        ...
        # Iteration 91.1.3: If data not ready, skip this iteration to prevent AI failure
        return prices_rsi
    else:
        # Iteration 91.1.3: Final Stability Fix - Console Only
    ...
    if not df_btc_1h.empty:
        # Iteration 91.1.8: Debugging BTC Price
    ...
    if not df_btc_ml.empty:
        ...
        # Iteration 91.1: DevOps Compliance - No more early return. 
        ...
        if is_pursuit_mode:
            ...
            print(f"🔥 [Iteration 91.1 | Final Stability Fix] 多頭追擊模式啟動 (BTC 24H Vol Change: {btc_vol_24h_change:.2%}, At High: {btc_at_high})")
        elif btc_vol_24h_change < 0:
            regime_mode = "震盪防禦"
            ml_threshold = 0.85
            min_rr = 1.3
            print(f"🛡️ [Iteration 91.1 | Final Stability Fix] 低量防禦模式啟動 (BTC 24H Vol Change: {btc_vol_24h_change:.2%})")
        else:
            print(f"🚀 [Iteration 91.1 | Final Stability Fix] 趨勢擴張模式 (BTC 24H Vol Change: {btc_vol_24h_change:.2%})")
    ...
    for symbol in symbols:
        try:
            ...
            if df_1h.empty:
                print(f"⚠️ [Iteration 91.1 | Final Stability Fix] {symbol} 1H data empty. Skipping.")
                continue
            ...
            if trend_mode:
                ...
                if aggressive_macd:
                    ...
                    if df['macd_line'].iloc[-1] > 0 and df['macd_signal'].iloc[-1] > 0 and macd_golden_cross:
                        ...
                        print(f"🔥 [Iteration 91.1 | Final Stability Fix] {symbol} MACD Aggressive Signal Detected!")
            ...
            # Iteration 91.1: High-Frequency & Confidence Ladder Logic
            ...
            print(f"🔍 [Iteration 91.1] {symbol} AI Score: {ai_score:.2%}, Signal: {long_signal}, Multiplier: {pos_size_multiplier}x")
                ...
                if last_state and last_state.get('status') == 'Closed' and last_state.get('exit_reason') in ['SL', 'SL_Trailing']:
                    ...
                    if (datetime.utcnow() - exit_time).total_seconds() < 1800: # 30 mins
                        ...
                        if latest['rsi'] >= last_state.get('entry_rsi', 0):
                            print(f"🛡️ [Iteration 91.1 | Final Stability Fix] {symbol} 處於止損保護期，且 RSI 未創新低。跳過進場。")
                            long_signal = False
            ...
            # Iteration 91.1: AI Score Calculation (DevOps Compliance)
            ...
            if not df_ml.empty and not df_btc_ml.empty:
                ...
                if len(df_ml) < 450:
                    print(f"⚠️ [Iteration 91.1 | DevOps Compliance] {symbol} Data insufficient (count: {len(df_ml)}/450). Skipping AI prediction.")
                    continue
                
                # Iteration 91.1: No more error masking with 0.5
                ...
                # Iteration 91.1: Raise error if data is missing
            ...
            if long_signal or short_signal:
                ...
                if side == 'LONG':
                    if not btc_sentiment_ok:
                        ...
                        # Iteration 91.1: No more continue. We must proceed to AI scoring.
                ...
                if ml_score > 0:
                    ...
                    # Iteration 91.1: Final Stability Fix | Laddered Logic
                ...
                if passed_filter:
                    # Iteration 91.1: Final check for Volume and Sentiment filters before adding to potential_signals
                ...
                if passed_filter:
                    ...
                    if ml_score >= 0.63:
                        ...
                        print(f"🛡️ [Iteration 91.1 | Final Stability Fix] {symbol} score {ml_score:.4f} but rejected: {reason}")
                    else:
                        print(f"🛡️ [AI Filter] {symbol} score {ml_score:.4f} < {ai_threshold}. Signal rejected.")
                    increment_ai_filtered_count()
        except Exception as e:
            print(f"❌ [Iteration 91.1 | DevOps Compliance] Error in strategy execution for {symbol}: {e}")
            import traceback
            traceback.print_exc()
    ...
    try:
        ...
        while True:
            try:
                ...
                # Iteration 91.1: 15-minute Heartbeat
                if (datetime.now() - last_report_time).total_seconds() >= 900:
                    ...
                    # Iteration 91.1: Final Stability Fix - Hourly Audit Silenced
                    ...
                    if not df_btc.empty:
                        ...
                        # Iteration 91.1.3: Calculate EMA200 for distance display
