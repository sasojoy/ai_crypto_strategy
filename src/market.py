...
def get_top_relative_strength_symbols():
    ...
    print(f"🎯 [Iteration 68.8 | Flash Sniper] Monitoring Selected Symbols: {selected_symbols}")
...
def check_upside_potential(symbol, entry_price, df_1h):
    ...
    try:
        ...
        if upside_pct < 0.012:
            print(f"🛡️ [Iteration 68.8 | Flash Sniper] [Space Check] {symbol} upside {upside_pct:.2%} < 1.2% to resistance ({recent_high:.2f}). Skipping.")
            return False
    except Exception as e:
...
def run_strategy(ml_model):
    ...
    if not df_btc_ml.empty:
        ...
        if btc_vol_24h_change < -0.20 and not (btc_bullish and btc_at_high):
            print(f"🚫 [Iteration 68.8 | Flash Sniper] 縮量進場禁止 (BTC 24H Vol Change: {btc_vol_24h_change:.2%})")
            return {}
        ...
        if is_pursuit_mode:
            ...
            print(f"🔥 [Iteration 68.8 | Flash Sniper] 多頭追擊模式啟動 (BTC 24H Vol Change: {btc_vol_24h_change:.2%}, At High: {btc_at_high})")
        elif btc_vol_24h_change < 0:
            regime_mode = "震盪防禦"
            ml_threshold = 0.85
            min_rr = 1.3
            print(f"🛡️ [Iteration 68.8 | Flash Sniper] 低量防禦模式啟動 (BTC 24H Vol Change: {btc_vol_24h_change:.2%})")
        else:
            print(f"🚀 [Iteration 68.8 | Flash Sniper] 趨勢擴張模式 (BTC 24H Vol Change: {btc_vol_24h_change:.2%})")
    ...
    for symbol in symbols:
        try:
            ...
            if df_1h.empty:
                print(f"⚠️ [Iteration 68.8 | Flash Sniper] {symbol} 1H data empty. Skipping.")
                continue
            ...
            if trend_mode:
                ...
                if aggressive_macd:
                    ...
                    if df['macd_line'].iloc[-1] > 0 and df['macd_signal'].iloc[-1] > 0 and macd_golden_cross:
                        ...
                        print(f"🔥 [Iteration 68.8 | Flash Sniper] {symbol} MACD Aggressive Signal Detected!")
                ...
                if last_state and last_state.get('status') == 'Closed' and last_state.get('exit_reason') in ['SL', 'SL_Trailing']:
                    ...
                    if (datetime.utcnow() - exit_time).total_seconds() < 1800: # 30 mins
                        ...
                        if latest['rsi'] >= last_state.get('entry_rsi', 0):
                            print(f"🛡️ [Iteration 68.8 | Flash Sniper] {symbol} 處於止損保護期，且 RSI 未創新低。跳過進場。")
                            long_signal = False
            ...
            if long_signal or short_signal:
                ...
                if side == 'LONG':
                    if not btc_sentiment_ok:
                        print(f"🚫 [Iteration 68.8 | Flash Sniper] {symbol} Long signal ignored: BTC Sentiment Bearish.")
                        continue
                    
                    if symbol in ['DOGE/USDT', 'XRP/USDT']:
                        funding_rate = fetch_funding_rate(symbol)
                        if funding_rate > 0.0005:
                            print(f"🚫 [Iteration 68.8 | Flash Sniper] {symbol} Long signal ignored: Funding Rate too high ({funding_rate*100:.4f}%).")
                            continue
                ...
                if not df_ml.empty and not df_btc_ml.empty:
                    ...
                    if not features.empty:
                        ...
                        if passed_filter:
                            print(f"🎯 [Iteration 68.8 | Flash Sniper] {symbol} {tier} Signal. Score: {ml_score:.4f}, Risk: {current_risk*100}%, RR: {target_rr}")
                            ...
                            if ml_score >= 0.63:
                                ...
                                print(f"🛡️ [Iteration 68.8 | Flash Sniper] {symbol} score {ml_score:.4f} but rejected: {reason}")
                            else:
...
def manage_positions(prices_rsi):
    ...
    for symbol in symbols:
        ...
        if not state.get('be_sl_active', False) and profit_pct >= 0.008:
            new_sl = entry_price
            print(f"🛡️ [Iteration 68.8 | Flash Sniper] {symbol} profit {profit_pct:.2%} >= 0.8%. Moving SL to Break-Even: {new_sl}")
            if update_sl_order(symbol, state.get('sl_order_id'), new_sl):
                state['be_sl_active'] = True
                state['sl_price'] = new_sl
                save_order_state(symbol, state)
                send_telegram_msg(f"🛡️ [Iteration 68.8 | Flash Sniper] {symbol} 已啟動保本止損 (Trailing to BE)。")
        ...
        if not state.get('partial_tp_done', False):
            if (side == 'LONG' and current_price >= rr_1_2_price) or (side == 'SHORT' and current_price <= rr_1_2_price):
                msg = f"💰 [Iteration 68.8 | Flash Sniper] {symbol} 達到 1.2 RR！執行 50% 減倉止盈。\n剩餘 50% 開啟 EMA 10 移動止損。"
                send_telegram_msg(msg)
        ...
        if state.get('partial_tp_done', False):
            ...
            if not df_exit.empty:
                ...
                if (side == 'LONG' and current_price < ema10) or (side == 'SHORT' and current_price > ema10):
                    msg = f"📈 [Iteration 68.8 | Flash Sniper] {symbol} 跌破 EMA 10！全數平倉獲利了結。"
                    send_telegram_msg(msg)
                    cancel_sl_order(symbol, state.get('sl_order_id'))
                    state['status'] = 'Closed'
        ...
        if not df_exit.empty:
            ...
            if side == 'LONG':
                ...
                if current_price >= bb_upper:
                    msg = f"🚀 [Iteration 68.8 | Flash Sniper] {symbol} 觸及布林上軌！全數平倉獲利了結。"
                    send_telegram_msg(msg)
                    cancel_sl_order(symbol, state.get('sl_order_id'))
                    state['status'] = 'Closed'
                    state['exit_price'] = current_price
        ...
        if (side == 'LONG' and current_price <= sl_price) or (side == 'SHORT' and current_price >= sl_price):
            msg = f"❌ [Iteration 68.8 | Flash Sniper] {symbol} 觸發止損！\n現價：{current_price:.2f} | 止損價：{sl_price:.2f}"
            send_telegram_msg(msg)
            cancel_sl_order(symbol, state.get('sl_order_id'))
            state['status'] = 'Closed'
            state['exit_price'] = current_price
        ...
        if (datetime.utcnow() - entry_time).total_seconds() >= 172800: # 48 hours
            if current_price > entry_price:
                msg = f"⏳ [Iteration 68.8 | Flash Sniper] {symbol} 持倉超過 48 小時且獲利為正，強行平倉釋放資金！"
                send_telegram_msg(msg)
                cancel_sl_order(symbol, state.get('sl_order_id'))
                state['status'] = 'Closed'
                state['exit_price'] = current_price
...
if __name__ == "__main__":
    ...
    STRATEGY_VERSION = "Iteration 68.8 | Final Sniper"
    last_heartbeat_time = 0
    last_summary_date = None
    
    # Iteration 68.8: Initialize ML Model at startup
    ...
    while True:
        try:
            ...
            # Iteration 68.8: 15-minute Heartbeat
