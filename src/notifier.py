...
def send_hourly_audit(equity, realized_pnl, active_positions):
    ...
    msg += (
        f"📈 帳戶總淨值: ${equity:,.2f}\n"
        f"----------------------------\n"
        f"狀態：🚀 【Iteration 85.2 | Elite Silent】"
    )
...
def send_rich_heartbeat(positions, scan_results, active_count, version, btc_status=None):
    ...
    # Iteration 85.2: Elite Silent - Simplified Layout & System Time
    ...
    for symbol, data in sorted_results:
        # Iteration 85.2: Simplified Symbol Report (AI > 55% Filter)
    ...
    msg += f"----------------------------\n"
    msg += f"版本: 🚀 【Iteration 85.2 | Elite Silent】"
